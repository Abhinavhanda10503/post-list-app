...
// Type for the saved post returned from the server
interface SavedPost {
  id: number;
  title: string;
  body: string;
...
interface PostFormProps {
  onPostSaved: (saved: SavedPost) => void;
}

function PostForm({ onPostSaved }: PostFormProps) {
  const { user } = useAuth();
  // user is guaranteed to be non‑null because this component is only rendered when logged in
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    try {
      const saved: SavedPost = await fetch('http://localhost:5000/local-posts', {
        method: 'POST',
    ...
    <div className="post-form-wrap">
      <div className="compose-row">
        ...
        <div className="compose-area">
          {!expanded ? (
            <button className="compose-trigger" onClick={() => setExpanded(true)}>
